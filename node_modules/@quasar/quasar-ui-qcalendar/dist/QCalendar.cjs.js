  ...
  function __renderCalendar () {
    const data = {
      ...
      class: {
        ...
        'q-calendar__vertical-stretch': props.verticalStretch === true
      }
    };
...
function useInterval (props, {
  ...
  const parsedIntervalStart = vue.computed(() => parseInt(props.intervalStart, 10));
  const parsedIntervalMinutes = vue.computed(() => parseInt(props.intervalMinutes, 10));
  const parsedIntervalCount = vue.computed(() => parseInt(props.intervalCount, 10));
  const parsedIntervalHeight = vue.computed(() => parseFloat(props.intervalHeight));
  ...
  const parsedStartMinute = vue.computed(() => parsedIntervalStart.value * parsedIntervalMinutes.value);
  const bodyHeight = vue.computed(() => parsedIntervalCount.value * parsedIntervalHeight.value);
  const bodyWidth = vue.computed(() => parsedIntervalCount.value * parsedCellWidth.value);

  const parsedWeekStart = vue.computed(() => startOfWeek(parsedStart.value));
  const parsedWeekEnd = vue.computed(() => endOfWeek(parsedEnd.value));
   ...
   */
  const ariaDateTimeFormatter = vue.computed(() => {
...
function useCellWidth (props) {
  const isSticky = vue.computed(() => props.cellWidth !== undefined);
...
function useKeyboard (props, {
  ...
  vue.watch(() => props.useNavigation, val => {
    if (val === true) {
      startNavigation();
    }
    else {
...
var QCalendarAgenda = vue.defineComponent({
  ...
  setup (props, { slots, emit, expose }) {
    const
      ...
      pane = vue.ref(null),
      headerColumnRef = vue.ref(null),
      focusRef = vue.ref(null),
      focusValue = vue.ref(null),
      datesRef = vue.ref({}),
      headDayEventsParentRef = vue.ref({}),
      headDayEventsChildRef = vue.ref({}),
      direction = vue.ref('next'),
      startDate = vue.ref(today()),
      endDate = vue.ref('0000-00-00'),
      maxDaysRendered = vue.ref(0),
    ...
    const parsedColumnCount = vue.computed(() => {
    ...
    const isLeftColumnOptionsValid = vue.computed(() => {
      return props.leftColumnOptions !== undefined && Array.isArray(props.leftColumnOptions)
    });

    const isRightColumnOptionsValid = vue.computed(() => {
    ...
    function __renderAgenda () {
      ...
      const agenda = vue.withDirectives(vue.h('div', {
...
var QCalendarDay = vue.defineComponent({
  name: 'QCalendarDay',
  ...
  setup (props, { slots, emit, expose }) {
    const
      ...
      size = vue.reactive({ width: 0, height: 0 }),
      dragOverHeadDayRef = vue.ref(false),
      dragOverInterval = vue.ref(false),
      // keep track of last seen start and end dates
      lastStart = vue.ref(null),
      lastEnd = vue.ref(null);
    ...
    const intervalsWidth = vue.computed(() => {
      if (rootRef.value) {
    ...
    function __renderHeadWeekday (day) {
      ...
      return vue.h('div', data, (slot && slot({ scope })) || __renderHeadWeekdayLabel(day, shortWeekdayLabel))
    }
    ...
    function __renderDayInterval (interval, columnIndex) {
      ...
      return vue.h('div', data, [ children, useFocusHelper() ])
    }
    ...
    function __renderBodyIntervals () {
      ...
      return vue.h('div', data, __renderIntervalLabels())
    }
    ...
    function __renderDaily () {
      ...
      const daily = vue.withDirectives(vue.h('div', {
        key: startDate.value,
...
function useMonth (props, emit, {
  ...
  const parsedMinWeeks = vue.computed(() => parseInt(props.minWeeks, 10));
  const parsedMinDays = vue.computed(() => parsedMinWeeks.value * props.weekdays.length);
  const parsedMonthStart = vue.computed(() => __getStartOfWeek(__getStartOfMonth(parsedStart.value)));
  const parsedMonthEnd = vue.computed(() => __getEndOfWeek(__getEndOfMonth(parsedEnd.value)));
  const parsedCellWidth = vue.computed(() => {
    let width = 0;
    if (props.cellWidth) {
      width = props.cellWidth;
  ...
  const parsedBreakpoint = vue.computed(() => {
  ...
  const parsedMonthLabelSize = vue.computed(() => {
...
var QCalendarMonth = vue.defineComponent({
  name: 'QCalendarMonth',
  ...
  setup (props, { slots, emit, expose }) {
    ...
    const workweekWidth = vue.computed(() => {
      if (rootRef.value) {
...
var QCalendarResource = vue.defineComponent({
  ...
  setup (props, { slots, emit, expose }) {
    const
      ...
      size = vue.reactive({ width: 0, height: 0 }),
      dragOverHeadDayRef = vue.ref(false),
      dragOverResource = vue.ref(false),
      dragOverResourceInterval = vue.ref(false),
      // keep track of last seen start and end dates
      lastStart = vue.ref(null),
      lastEnd = vue.ref(null);
    ...
    const parsedResourceHeight = vue.computed(() => {
    ...
    const parsedResourceMinHeight = vue.computed(() => {
      return parseInt(props.resourceMinHeight, 10)
    });

    const parsedIntervalHeaderHeight = vue.computed(() => {
    ...
    function __renderResourcesError () {
      return vue.h('div', {}, 'No resources have been defined')
    }
    ...
    function __renderBodyResources () {
      ...
      return vue.h('div', data, __renderResources())
    }
    ...
    function __renderResource () {
      ...
      const resource = vue.withDirectives(vue.h('div', {
...
var QCalendarScheduler = vue.defineComponent({
  ...
  setup (props, { slots, emit, expose }) {
    ...
    const resourcesWidth = vue.computed(() => {
      if (rootRef.value) {
    ...
    function __renderScheduler () {
      ...
      const scheduler = vue.withDirectives(vue.h('div', {
        key: startDate.value,
...
var QCalendar = vue.defineComponent({
  name: 'QCalendar',
  props: {
    mode: {
      type: String,
  ...
  setup (props, { attrs, slots, expose }) {
    ...
    return () => vue.h(component.value, { ref: calendar, ...props, ...attrs }, slots)
  }
});
...
exports.QCalendarAgenda = QCalendarAgenda;
exports.QCalendarDay = QCalendarDay;
exports.QCalendarMonth = QCalendarMonth;
exports.QCalendarResource = QCalendarResource;
exports.QCalendarScheduler = QCalendarScheduler;
